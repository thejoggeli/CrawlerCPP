...
bool NAU7802::begin(I2CDevice* i2c, bool initialize)
{
  //Get user's options
  this->i2c = i2c;
...
bool NAU7802::waitForCalibrateAFE(uint64_t timeout_ms)
{
  uint64_t begin = Time::GetSystemTimeMicros();
  NAU7802_Cal_Status cal_ready;

  while ((cal_ready = calAFEStatus()) == NAU7802_CAL_IN_PROGRESS)
  {
    if ((timeout_ms > 0) && ((Time::GetSystemTimeMicros() - begin) > timeout_ms))
    {
      break;
    }
...
int32_t NAU7802::getReading()
{
  ...
  uint8_t data[3];
  if(i2c_read(i2c, NAU7802_ADCO_B2, data, 3) == -1){
    return 0;
  }
...
int32_t NAU7802::getAverage(uint8_t averageAmount)
{
  ...
  uint64_t startTime = Time::GetSystemTimeMicros();
  while (1)
  {
    if (available() == true)
    {
    ...
    if (Time::GetSystemTimeMicros() - startTime > 1000)
...
{
  uint8_t data;
  if(i2c_read(i2c, registerAddress, &data, 1) == -1){
    return -1;
  }
  return data;
}
...
{
  if(i2c_write(i2c, registerAddress, &value, 1) == -1){
    return false;
  }
  return true;
}

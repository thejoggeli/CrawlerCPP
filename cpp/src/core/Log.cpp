...
void LogInfo(LogConcator& log){ 
	std::lock_guard<std::mutex> lock(mtx);
	LogInner(LogLevel::LOG_INFO, "Info", log.stringStream.str()); 	
}
void LogInfo(const std::string& str){ 
	std::lock_guard<std::mutex> lock(mtx);
	LogInner(LogLevel::LOG_INFO, "Info", str); 
}
...
void LogDebug(LogConcator& log){ 
	std::lock_guard<std::mutex> lock(mtx);
	LogInner(LogLevel::LOG_DEBUG, "Debug", log.stringStream.str()); 
}
void LogDebug(const std::string& str){ 
	std::lock_guard<std::mutex> lock(mtx);
	LogInner(LogLevel::LOG_DEBUG, "Debug", str); 
}
...
void LogError(LogConcator& log){
	std::lock_guard<std::mutex> lock(mtx);
	LogInner(LogLevel::LOG_INFO, "Error", log.stringStream.str()); 
}
void LogError(const std::string& str){ 
	std::lock_guard<std::mutex> lock(mtx);
	LogInner(LogLevel::LOG_INFO, "Error", str); 
}

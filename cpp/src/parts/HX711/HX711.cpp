...
void HX711::begin(uint8_t dataPin, uint8_t clockPin)
{
  ...
  Crawler::LogDebug("HX711", iLog << "begin(dataPin=" << (int) dataPin << ", clockPin=" << (int) clockPin << ")");

  GPIO::setup(_dataPin, GPIO::IN);
  GPIO::setup(_clockPin, GPIO::OUT);
  GPIO::output(_clockPin, GPIO::LOW);
...
void HX711::yield(){
  Crawler::Time::SleepMicros(1);
}

bool HX711::is_ready()
{
  return GPIO::input(_dataPin) == GPIO::LOW;
}


//  From datasheet page 4
//  When output data is not ready for retrieval,
//       digital output pin DOUT is GPIO::HIGH.
//  Serial clock input PD_SCK should be GPIO::LOW.
//  When DOUT goes to GPIO::LOW, it indicates data is ready for retrieval.
float HX711::read()
{
  //  this BLOCKING wait takes most time...
  while (GPIO::input(_dataPin) == GPIO::HIGH) yield();
  ...
  while (m > 0)
  {
    ...
    GPIO::output(_clockPin, GPIO::HIGH);
    Crawler::Time::SleepMicros(1);
    GPIO::output(_clockPin, GPIO::LOW);
    Crawler::Time::SleepMicros(1);
    m--;
  ...
  Crawler::LogDebug("HX711", iLog << "read " << v.value);

  _lastRead = Crawler::Time::GetSystemTimeMicros();
  return 1.0 * v.value;
}
...
void HX711::wait_ready(uint64_t micros)
{
  while (!is_ready())
  {
    Crawler::Time::SleepMicros(micros);
  }
}


bool HX711::wait_ready_retry(uint8_t retries, uint64_t micros)
{
  while (retries--)
  {
...
bool HX711::wait_ready_timeout(uint64_t timeout, uint64_t micros)
{
  uint64_t start = Crawler::Time::GetSystemTimeMicros();
  while (Crawler::Time::GetSystemTimeMicros() - start < timeout)
  {
    if (is_ready()) return true;
    Crawler::Time::SleepMicros(micros);
  }
...
{
  //  at least 60 us GPIO::HIGH
  GPIO::output(_clockPin, GPIO::HIGH);
  Crawler::Time::SleepMicros(64);
}
...
uint8_t HX711::_shiftIn()
{
  ...
  while (mask > 0)
  {
    ...
    Crawler::Time::SleepMicros(1);   //  T2  >= 0.2 us
    if (GPIO::input(data) == GPIO::HIGH)
    {
      value |= mask;
    }
    GPIO::output(clk, GPIO::LOW);
    Crawler::Time::SleepMicros(1);   //  keep duty cycle ~50%
    mask >>= 1;
  }
  return value;
}

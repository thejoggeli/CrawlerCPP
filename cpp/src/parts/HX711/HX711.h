...
public:
  ...
  //  wait until ready,
  //  check every ms
  void     wait_ready(uint64_t micros = 0);
  //  max # retries
  bool     wait_ready_retry(uint8_t retries = 3, uint64_t micros = 0);
  //  max timeout
  bool     wait_ready_timeout(uint64_t timeout = 1000, uint64_t micros = 0);
  ...
  //  TIME OF LAST READ
  uint64_t last_read()                  { return _lastRead; };
